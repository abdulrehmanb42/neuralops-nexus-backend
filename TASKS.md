...
| 170 | Fat docker image for easy self-host distribution | Bundle nucleus + nexus-ai + nginx + redis + celery + centrifugo/transport into one image (supervisord or similar) so end users only need docker-compose.yml + a few pulled images (fat app, postgres, chromadb). Tailscale (if added) should be a separate sidecar service in the same compose file, not merged into the fat image — keeps its elevated NET_ADMIN/NET_RAW privileges isolated from app code |
