# NeuralOps Backend API Catalog (v2)

> **Scope note.** This is the deeper, source-derived catalog: it adds the Django
> model reference (§0), the AI/LLM configuration layer (§1), full request/response
> field specs on every endpoint, and the Scheduling API (§8). It sits alongside
> the original `neuralops-backend-api-catalog.md`, which is still the place to
> look for the **MCP tool server** inventory (SerpAPI / Odoo / filesystem tool
> names), the device-activation endpoints (`GET /auth/init/`, `GET /auth/status/`),
> `nexus-ai`'s verify surface (`POST /internal/models/verify`,
> `POST /internal/agents/verify`, `GET /internal/providers`, `/`, `/health`), and
> the COMPANY-vs-PROJECT permission-scope prose for the AI Intelligence resources.
...
| **Vector store** | `chroma` (ChromaDB, own container) | `VECTOR_STORE` env var | Verified against source: `config.py` sets `VECTOR_STORE: str = "chroma"`, `chromadb` is in `docker/dev/requirements.txt`, and `apps/implementations/vectorstore/chroma_store.py` is the only backend actually present. `pgvector` and `qdrant` are declared switch cases in `VectorStoreFactory` but **neither `pgvector_store.py` nor `qdrant_store.py` exists** — selecting either fails at import. |

**Vector store history — read this before "fixing" anything.** A `pgvector` backend was
built and trialled (a stale `pgvector_store.cpython-311.pyc` still sits in
`apps/implementations/vectorstore/__pycache__/` as the only trace), but it did not
work out and the project **reverted to ChromaDB**. So `readme.md`'s "Architecture at
a glance" table listing `chromadb` as the vector store is **correct and current** —
an earlier draft of this catalog flagged it as stale in favour of `pgvector`; that
flag was wrong and has been removed. Do not change it back.
...
> The COMPANY-vs-PROJECT scope rules behind these rights (why `ai_model.create`/`delete`
> are company-only but `ai_model.attach` is project-scoped, and how row-visibility
> filtering replaces blanket 403s on list endpoints) are written out in
> `neuralops-backend-api-catalog.md` §7 and `authn/permissions/USE_CASES.md` UC13–UC17.
...
FastAPI app (`apps/main.py`), base path `/api/v1/`, auth is `X-Internal-Key` header — this service is **never** called directly by the frontend, only by nucleus, and has no Django models of its own (it's stateless — everything it needs comes from §9). Its two main routers:
...
> `nexus-ai` also serves a small unauthenticated verify/health surface not covered
> here — `POST /api/v1/internal/models/verify`, `POST /api/v1/internal/agents/verify`,
> `GET /api/v1/internal/providers`, `GET /`, `GET /health`. See
> `neuralops-backend-api-catalog.md` §9.
